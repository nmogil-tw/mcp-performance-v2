...
function displaySessions() {
    ...
    tbody.innerHTML = filteredSessions
        .sort((a, b) => {
            // Sort by directory ID first, then by task, then by mode
            const dirA = a.directoryId || 'Unknown';
            const dirB = b.directoryId || 'Unknown';
        ...
        .map(s => `
            <tr>
                <td><strong>${s.directoryId || 'Unknown'}</strong></td>
